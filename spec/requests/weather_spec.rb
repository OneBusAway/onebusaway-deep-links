...
describe "Api::V1::WeatherController", type: :request do
  ...
  def expect_response_to_offer_weather_for_location(lat,lon)
    expect(json.keys).to eq(["latitude", "longitude", "region_identifier", "region_name", "retrieved_at", "units", 
                             "today_summary", "current_forecast", "hourly_forecast"])
    ...
    expect(json['current_forecast'].keys).to eq(["icon", "precip_per_hour", "precip_probability", "summary", 
                                                 "temperature", "temperature_feels_like", "time", "wind_speed"])
