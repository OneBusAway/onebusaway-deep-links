class Admins::AlarmsController < ApplicationController
  ...
  def show
    ...
    @arr_dep = Rails.cache.fetch(key, expires_in: 30.seconds) do
      server.arrival_and_departure(stop_id: @alarm.stop_id, service_date: @alarm.service_date, 
                                   stop_sequence: @alarm.stop_sequence, trip_id: @alarm.trip_id, vehicle_id: @alarm.vehicle_id)
    end
  end
end
